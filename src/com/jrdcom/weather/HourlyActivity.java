...
public class HourlyActivity extends Activity {
    ...
    private ServiceConnection conn = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            if (myService != null) {
                ...
                mListView.setDivider(HourlyActivity.this.getResources().getDrawable(
                        R.drawable.line_h));
                mListView.setDividerHeight(1);

                // PR351549-Feng.Zhuang-001 Modify begin
                /*
                 * if (isWifiConnected || isMobileConnected || isOtherConnected) { // PR684354-Neo
                 * Skunkworks-kehao.wei-001 modify begin try { requestAd(); } catch (Exception e) {
                 * Log.e(TAG, "request Ad failed : " + e.toString()); } // PR684354-Neo
                 * Skunkworks-kehao.wei-001 modify end }
                 */
            }
        }
    };
    ...
    private void requestAd() {
        ...
        try {
            strUserAgent = "strUserAgent="
                    + URLEncoder.encode(mWebSettings.getUserAgentString(), "utf-8") + "&";
    ...
    private ArrayList<HashMap<String, Object>> getData() {
        ...
        for (int i = 0; i < nHoursSize && i < 24; i++) {
            ...
            String imagepath = "normal_widget_icons_"
                    + (Integer.parseInt(hours.get(i).getIcon()) < 10 ? "0" : "")
                    + hours.get(i).getIcon();
            ...
            int timeType = getTimeType(hours.get(i).getTime(),
                    Integer.parseInt(hours.get(i).getIcon().trim()));
            int background = 0;
            int fontColor = 0;
            switch (timeType) {
                case DAYTIME:
    ...
    private String getWeatherText(String weatherID) {
        ...
        String[] weatherDescriptions = this.getResources()
                .getStringArray(R.array.weather_icon_desc);

        return weatherDescriptions[iconID];
    }

    private String getWeekly(String sWeek) {
        String[] enWeekly = new String[] {
                "Mon", "Tue", "Wed", "Thu", "Fri",
                "Sat", "Sun"
        };
        if (sWeek != null) {
