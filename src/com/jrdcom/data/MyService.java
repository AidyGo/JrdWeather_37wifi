...
public class MyService extends Service {
    ...
    // private static final String URL_CITY_FIND =
    // "http://tclandroidicsapp.accu-weather.com/widget/tclandroidicsapp/city-find.asp?";
    // private static final String URL_WEATHER_DATA =
    // "http://tclandroidicsapp.accu-weather.com/widget/tclandroidicsapp/weather-data.asp?location=";
    ...
    private static final String CITY_TOKYO_LOCATIONKEY = "226396";// add by jielong.xing at
                                                                  // 2014-09-30
    ...
    private String mSearchURLNoId;// modify by shenxin for get different language for city name
                                  // PR458505
    ...
    // private double mPositionData[][] =
    // {{33.7988684,-79.3151525},{43.7697581,-79.3716889},{23.0376432, 114.3430633}, {32.6555757,
    // -64.6091765}, {48.6461768, -79.6417216}};
    ...
    Handler mHandler = new Handler()
    {
        public void handleMessage(Message msg) {
            switch (msg.what)
            {
                ...
                // Add by Wells.Tang 2014-9-25,for requestLocationUpdate need to run on ui thread
                // and must
    ...
    {
        /*
         * if(mHandler!=null) { mHandler.removeMessages(MSGUPDATEAUTOLOCATE); }
         */
        mAutoUpdate = false;
    }
        ...
        if (location != null)
        {
            try
            {
                ...
                if (city != null)
                {
                    ...
                    {
                        if ((lastAutoLocateTime != -1 && currentTime - lastAutoLocateTime >= ONEHOUR)
                                || forceRefresh)
                        {
                            city.setAutoLocate(true);
                            if (oldCity != null)
                    ...
                    if (ret)
                    {
                        SharePreferenceUtils.saveLong(MyService.this, KEY_LAST_AUTOLOCATETIME,
    ...
    public void onCreate() {
        ...
        SharedPreferences pref = getApplicationContext().getSharedPreferences("isFirstUse",
    ...
    public void onDestroy() {
        ...
        /*
         * try { if(mLocationManager!=null) { if(mRequestUpdateSuccess) {
         * mLocationManager.removeUpdates(this); } mLocationManager = null; } } catch(Exception e) {
         * }
         */
        mHandler.sendEmptyMessage(MSGREMOVELOCATIONUPDATE);
    ...
    public boolean requestLocationUpdate()
    {
        ...
        updateConnectedFlags();
        Log.d(TAG, "requestLocationUpdate: mRequestUpdateSuccess=" + mRequestUpdateSuccess
                + ", isScreenOn = " + isScreenOn + ", mLocationManager!=null?"
                + (mLocationManager != null) + ", isGoogleServiceOpened = " + isGoogleServiceOpened);
        if (!mRequestUpdateSuccess && isScreenOn && mLocationManager != null
                && isGoogleServiceOpened)
        {
            ...
            try
            {
                ...
                if (isGPSProvideEnable) {
                    mLocationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 10000, 1,
                            mGpsListener);
                ...
                Log.d("jielong", "requestLocationUpdate:isNetworkRequestOpen = "
                        + isNetworkRequestOpen + ", isGPSRequestOpen = " + isGPSRequestOpen);
            } catch (Exception e)
            {
                noError = false;
    ...
    public void removeLocationUpdate()
    {
        if (mRequestUpdateSuccess)
        {
            ...
            Log.d("jielong", "removeLocationUpdate start:isNetworkRequestOpen = "
                    + isNetworkRequestOpen + ", isGPSRequestOpen = " + isGPSRequestOpen);
            try
            {
                // zhaoyun.wu begin
                ...
                mLocationUpdated = false;
                Log.d("jielong", "removeLocationUpdate end:isNetworkRequestOpen = "
                        + isNetworkRequestOpen + ", isGPSRequestOpen = " + isGPSRequestOpen);
            } catch (Exception e)
            {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // }
        /*
         * else { updateConnectedFlags(); if ((isWifiConnected || isMobileConnected ||
         * isOtherConnected)) { mAutoUpdate = true; updateWeather(); } }
         */
    ...
    private void updateConnectedFlags() {
         ...
         */
        isGoogleServiceOpened = (Settings.Secure.getInt(getContentResolver(),
    ...
    public void sendCityFindRequest(String cityName) {
        Log.i("aidy", "sendCityFindRequest() -- cityName = " + cityName);
    ...
    private class CityFindThread extends Thread {
        ...
        public void run() {
            try {
                List<City> tempCityList = CityFindRequest.findCityByName(
                        URLEncoder.encode(cityName, "utf-8"), getLanguage(), true);
                if (tempCityList.size() != 0) {
                    mCityList = tempCityList;
                }
                Intent intent = new Intent(LocateActivity.CITY_BROADCAST);
                intent.putExtra("city", tempCityList.size() != 0);
                sendBroadcast(intent);
            } catch (Exception e) {
                Log.e(TAG, "CityFindThread:" + e.getMessage());
                Intent it = new Intent(LocateActivity.WEATHER_BROADCAST);
    ...
    public void autoLocate(double latitude, double longitude) {
        /*
         * String location = "latitude=" + latitude + "&longitude=" + longitude; int languageId =
         * getLanguageId(); mSearchURL = (URL_CITY_FIND + location + "&langid=" +
         * languageId).trim();//modify by shenxin for get different language for city name PR458505
    ...
    private boolean isUseProvinceNameAsLocationName()
    {
        ...
        return CustomizeUtils
                .getBoolean(MyService.this, "def_weather_use_province_as_locationname");
    }
    ...
    private City findCity(double latitude, double longitude)
    {
        ...
        try {
            /*
             * InputStream mInputStream = downloadUrl(mSearchURL).getInputStream(); List<City> citys
             * = CityFindParser.parse(mInputStream); mInputStream.close();
             */
            ...
            if (citys.size() != 0) {
                ...
                if (TextUtils.isEmpty(city.getCityName())) {
                     ...
                     */
                    List<City> mCitys = CityFindRequest
                            .findCityByGeoLocation(location, lang, false);
                    if (mCitys.size() != 0) {
                        city = mCitys.get(0);
                    }
                }
    ...
    class CityFindGPSThread extends Thread {
        ...
        public void run() {
            try {
                ...
                List<City> citys = CityFindRequest.findCityByGeoLocation(location, lang, true);
                /*
                 * InputStream mInputStream = downloadUrl(mSearchURL).getInputStream(); List<City>
                 * citys = CityFindParser.parse(mInputStream); mInputStream.close();
                 */
                ...
                if (citys.size() != 0) {
                    ...
                    if (TextUtils.isEmpty(city.getCityName())) {
                        /*
                         * InputStream mNoIdInputStream =
                         * downloadUrl(mSearchURLNoId).getInputStream(); List<City> mCitys =
                         * CityFindParser.parse(mNoIdInputStream); mNoIdInputStream.close();
                         */
                        List<City> mCitys = CityFindRequest.findCityByGeoLocation(location, lang,
                                false);
                        if (mCitys.size() != 0) {
                            city = mCitys.get(0);
                        }
    ...
    public boolean insertCityBlock(String locationkey, boolean manu)
    {
        ...
        try {
            ...
            /*
             * InputStream mInputStream = downloadUrl(mWeatherURL).getInputStream(); Weather weather
             * = new WeatherDataParser().parse(mInputStream); mInputStream.close();
             */
    ...
    private class WeatherDataThread extends Thread {
        ...
        public void run() {
            try {
                ...
                /*
                 * InputStream mInputStream = downloadUrl(mWeatherURL).getInputStream(); //reduce
                 * one time http post Weather weather = new WeatherDataParser().parse(mInputStream);
                 * mInputStream.close();
                 */
                String lang = getLanguage();
                Weather weather = getWeather(locationKey, lang);
    ...
    public void checkUpdate() {
        boolean flag = false;
        updateConnectedFlags();
        Log.e(TAG, "checkUpdate::::isWifiConnected = " + isWifiConnected + ", isMobileConnected = "
                + isMobileConnected + ", isOtherConnected = " + isOtherConnected
                + ", isScreenOn = " + isScreenOn + ", isGoogleServiceOpened = "
                + isGoogleServiceOpened);
        if ((isWifiConnected || isMobileConnected || isOtherConnected) && isScreenOn)
        {
            ...
            if (citys != null && citys.size() != 0) {
                long lastRefreshTime = SharePreferenceUtils.getLong(MyService.this,
                        KEY_LAST_REFRESHTIME, -1);
                ...
                else if (isGoogleServiceOpened)
                {
                    ...
                    autoLocateTime = SharePreferenceUtils.getLong(MyService.this,
                            KEY_LAST_AUTOLOCATETIME, -1);

                    if (autoLocateTime == -1 || time - autoLocateTime > HALFHOUR)
                    {
                        Log.e(TAG,
                                "checkUpdate::::time more than halfhour, mRequestUpdateSuccess == "
                                        + mRequestUpdateSuccess);
                        // if location update is ongoing, don't duplicate do the update action.
                        if (mRequestUpdateSuccess) {
                            return;
                        }
                        new Thread()
                        {
                            ...
                            {
                                /*
                                 * boolean scanWifi = startScanWifi(); //sleep 5 seconds to wait for
                                 * the wifi scan result if(scanWifi) { try { Thread.sleep(5000); }
                                 * catch(Exception e) { } }
                                 */
                                ...
                                {
                                    Log.e(TAG,
                                            "checkUpdate::::wait 180 seconds,but the location has not updated");
     ...
     */
    /*
     * private boolean startScanWifi() { //Log.d(TAG,"startScanWifi"); boolean ret = false; try {
     * int wifiScanAways =Settings.Global.getInt(getContentResolver(), "wifi_scan_always_enabled",
     * 0); if( wifiScanAways ==1) { if(mWifiManager == null) { mWifiManager =
     * (WifiManager)getSystemService(Context.WIFI_SERVICE); } if(mWifiManager!=null) {
     * mWifiManager.startScan(); ret = true; } } } catch (SettingNotFoundException e) { // TODO
     * Auto-generated catch block Log.e(TAG,"settings not found"); e.printStackTrace(); } return
     * ret; }
     */
    ...
    private class WeatherUpdateThread extends Thread {
        ...
        public void run() {
            try {
                ...
                Log.d(TAG, "WeatherUpdateThread:::isWifiConnected = " + isWifiConnected
                        + ", isMobileConnected = " + isMobileConnected + ", isOtherConnected = "
                        + isOtherConnected + ", isScreenOn = " + isScreenOn
                        + ", isGoogleServiceOpened = " + isGoogleServiceOpened);
                if (isGoogleServiceOpened)
                {
                    /*
                     * boolean scanWifi = startScanWifi(); if(scanWifi) { try { Thread.sleep(5000);
                     * } catch(Exception e) { } }
                     */
                    mHandler.sendEmptyMessage(MSGREQUESTLOCATIONUPDATE);
                    try
                    ...
                    {
                        Log.e(TAG,
                                "WeatherUpdateThread:::wait 180 seconds,but the location has not updated");
                        if (!mAutoUpdate)
                        {
                            Intent it = new Intent(LocateActivity.WEATHER_BROADCAST);
                            it.putExtra("connect_timeout", true);
                    ...
                    else
                    {
                        ...
                        Log.e(TAG, "WeatherUpdateThread:::latitude = " + latitude
                                + ", longitude = " + longitude);
                    }
                }
                // modify end
                ...
                Log.e(TAG, "WeatherUpdateThread:::mAutoUpdate = " + mAutoUpdate
                        + ", updateWeatherOnly = " + updateWeatherOnly);
                ...
                for (int i = 0; i < citySize; i++) {
                    ...
                    if (updateWeatherOnly || !isAutoLocate) {
                        /*
                         * mWeatherURL = new
                         * StringBuilder(URL_WEATHER_DATA).append(locationKey).toString().trim();
                ...
                {
                    /*
                     * if (true) { int len = mPositionData.length; java.util.Random rand = new
                     * java.util.Random(); int r = rand.nextInt(len); double position[] =
                     * mPositionData[r]; latitude = position[0]; longitude = position[1];
                    ...
                    if (city != null)
                    {
                        ...
                        if (autolocateCity != null)
                        {
                            ...
                            // new autolocate city and old autolocate city is in the same region,use
                            // the same locationkey
                            else
                            {
                ...
                if (cityList != null && cityList.size() > 0) { // PR 466448 - Neo Skunkworks - Tom
                                                               // Yu - 001
                ...
                SharePreferenceUtils.saveLong(MyService.this, KEY_LAST_REFRESHTIME,
                        System.currentTimeMillis());

                /*
                 * if(mAutoUpdate && !updateWeatherOnly && (isWifiConnected|| isMobileConnected ||
                 * isOtherConnected)) { if(!autolocateSuccess) { mCurrentTime = 0;
    ...
    private void insertWeatherIntoDB(Weather weather, boolean manu) {
        ...
        Intent i = new Intent(LocateActivity.WEATHER_BROADCAST);
        ...
        long lastRefreshTime = SharePreferenceUtils.getLong(MyService.this, KEY_LAST_REFRESHTIME,
                -1);
    ...
    public void insertCity(City city, boolean isAutoLocate) {// add a boolean parameter for
                                                             // autolocate
    ...
    /*
     * private int getLanguageId() { int languageId = 0; String langcode =
     * Locale.getDefault().getLanguage() + "_" + Locale.getDefault().getCountry(); String[] langids
     * = this.getResources().getStringArray(R.array.language); PR 560936- Neo Skunkworks - Wells
     * Tang - 001 Begin for (int i = 0; i < langids.length; i++) { if
     * (langcode.contains(langids[i])) { languageId = i; } } languageId ++; PR 560936- Neo
     * Skunkworks - Wells Tang - 001 End return languageId; }
     */
    ...
    private synchronized void sendTokyoFindRequest(String cityName) {
        /*
         * int languageId = getLanguageId(); try { mSearchURL = new StringBuilder(URL_CITY_FIND)
         * .append("location=" + URLEncoder.encode(cityName, "utf-8") + "&langid=" +
         * languageId).toString().trim(); mWeatherURL = new
         * StringBuilder(URL_WEATHER_DATA).append(CITY_TOKYO_LOCATIONKEY).toString().trim(); } catch
         * (Exception e) { Log.e(TAG, "findTokyoData exception :: " + e.getMessage()); }
         */
    ...
    private class TokyoCityAndWeatherFindThread extends Thread {
        ...
        public void run() {
            try {
                /*
                 * InputStream mInputStream = downloadUrl(mSearchURL).getInputStream(); List<City>
                 * tempCityList = CityFindParser.parse(mInputStream); mInputStream.close();
                 */
                List<City> tempCityList = CityFindRequest.findCityByName(
                        URLEncoder.encode(cityName, "utf-8"), getLanguage(), true);
                ...
                if (tokyoCity != null) {
                    ...
                    SharePreferenceUtils.checkCommonCity(getApplicationContext(),
                            tokyoCity.getLocationKey());

                    /*
                     * mInputStream = downloadUrl(mWeatherURL).getInputStream(); Weather weather =
                     * new WeatherDataParser().parse(mInputStream); mInputStream.close();
                     */
                    /*
                    ...
                    Editor editor = getApplicationContext().getSharedPreferences("isFirstUse",
    ...
    private boolean isForceSetTokyo() {
        return CustomizeUtils.getBoolean(MyService.this,
                "def_weather_forceSetTokyoAsDefaultCity_on");
    }
    ...
    private Weather getWeather(String locationKey, String lang) throws ClientProtocolException,
